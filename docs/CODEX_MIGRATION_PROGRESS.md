...
- Phase 25 밴드 세션 모집 관리: 등록/수정/삭제 1차 API 연동 완료
...
- 팬 마이페이지에 `/fan/my/concert-alert`, `/fan/my/live-alert` 알림 설정 화면을 추가했다.
- 알림 설정 조회 `/users/me/notification-settings?mode=FAN`와 설정 변경 `/users/me/notification-settings/{settingType}` API를 연결했다.
- 팬 탐색 화면에 밴드 검색어 입력, 인기순/최신순 정렬, 장르/지역 필터를 추가했다.
- 팬 탐색 밴드 검색 API `/explore/search?type=BAND`를 연결하고 기존 팔로우/프로필 이동 동작을 검색 결과에서도 재사용했다.
- 밴드 홈 콘텐츠 상세 `/band/home/contents/[postId]`와 공연 상세 `/band/home/concerts/[performanceId]` route를 추가했다.
- 밴드 홈 콘텐츠/공연 상세 조회 API와 삭제 mutation을 연결했다.
- 밴드 홈 콘텐츠/일정 목록에서 상세 이동과 삭제 확인 액션을 연결했다.
...
- 밴드 마이 탭을 실제 요약/메뉴 화면으로 교체했다.
- 밴드 모집 공고 알림 설정 `/band/my/recruit-alert`와 라이브 알림 설정 `/band/my/live-alert` 화면을 추가했다.
- 밴드 알림 설정 조회/변경을 `/users/me/notification-settings?mode=BAND`, `/users/me/notification-settings/{settingType}` API에 연결했다.
...
- 밴드 세션 모집 등록/수정 route `/band/session/recruitments/form`을 추가했다.
- 세션 모집 등록 `/sessions/recruitments`, 수정 `/sessions/recruitments/{sessionRecruitmentId}`, 수정 정보 `/sessions/recruitments/{sessionRecruitmentId}/edit`, 삭제 `/sessions/recruitments/{sessionRecruitmentId}` API를 연결했다.
- 밴드 세션 목록과 상세 화면에서 모집 공고 등록/수정/삭제 진입을 연결했다.
...
- 팬 홈, 공연 목록/상세, 팬 탐색/밴드 프로필, 팬 탐색 팔로우/언팔로우/밴드/공연/콘텐츠 검색/콘텐츠 상세, 공연 관심/알림 mutation, 팬/밴드 마이페이지 알림 설정, 팬 마이페이지/세부 목록, Onboarding 약관 데이터, Expo Notifications 권한 요청, 밴드 홈 세부 조회 API, 콘텐츠/공연 상세/삭제/등록/수정, 밴드 세션 모집 목록/상세/등록/수정/삭제/지원하기, 받은 지원 목록을 구현한 상태. 다음은 지원자 상세 API 연동이다.

## 다음에 해야 할 작업

1. 밴드 세션 지원자 상세 API를 연결한다.
...
- 팬 콘텐츠 상세/좋아요/댓글 endpoint는 웹과 동일하게 `/posts/{postId}/detail`, `/posts/{postId}/likes`, `/posts/{postId}/comments`를 사용한다.
...
- 알림 설정 endpoint는 웹과 동일하게 `/users/me/notification-settings`, `/users/me/notification-settings/{settingType}`을 사용하고 mode `FAN`/`BAND`를 구분한다.
...
- 밴드 세션 모집 endpoint는 웹과 동일하게 `/sessions/recruitments`, `/sessions/recruitments/{sessionRecruitmentId}`, `/sessions/recruitments/{sessionRecruitmentId}/edit`, `/sessions/recruitments/{sessionRecruitmentId}/interest`를 사용한다.
- 밴드 세션 모집 등록 payload의 `bandMemberId`는 현재 모바일 밴드 마이 요약의 `bandMemberProfileId`를 사용한다. 실제 백엔드 응답/권한 검증에서 `bandMemberId`와 profile id 매핑 확인이 필요하다.
...
- `/fan/explore`: 팬 탐색 탭
- `/fan/explore/contents/[postId]`: 팬 콘텐츠 상세/좋아요/댓글
...
- `/fan/my/concert-alert`: 팬 공연 알림 설정
- `/fan/my/live-alert`: 팬 라이브 알림 설정
- `/band/home`: 밴드 홈 탭
...
- `/band/my/applications`: 밴드 받은 지원 관리
- `/band/session`: 밴드 세션 탭
- `/band/session/recruitments/[recruitmentId]`: 밴드 세션 모집 공고 상세
- `/band/session/recruitments/form`: 밴드 세션 모집 공고 등록/수정
- `/band/live`: 밴드 라이브 탭
- `/band/my`: 밴드 마이 탭
...
- 밴드 세션은 모집 목록/상세/등록/수정/삭제/관심 토글/지원하기/받은 지원 목록을 완료했다. 지원서 작성, 지원자 상세/수락/거절, 세션 찾기, 모집 공고 페이지네이션은 아직 웹 parity 전이다.
...
- 팬 콘텐츠 상세 `/posts/{postId}/detail`, 좋아요, 댓글 실제 API 상태 동기화
...
- 팬 알림 설정 조회/변경 실제 API 상태 동기화
- 밴드 알림 설정 조회/변경 실제 API 상태 동기화
